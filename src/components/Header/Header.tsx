...
export const Header = ({ toggleNavigation }: HeaderProps) => {
	...
	return (
		<>
			<AppBar position='fixed' style={{ background: '#05141F', paddingTop: "1rem", paddingBottom: "1rem" }} sx={{ zIndex: (theme) => theme.zIndex.drawer + 1 }}>
				<Toolbar disableGutters variant='dense'>
					{/* <Hamburger toggleNavigation={toggleNavigation} /> */}
					<Box sx={{ flexGrow: 1 }} />
					<Box sx={{ display: { xs: 'none', md: 'flex', alignItems: 'center' } }}>
						<Button color="inherit" sx={{ mr: 2 }}>The Collection</Button>
						<Button color="inherit" sx={{ mr: 2 }}>Official Rules</Button>
						<Button color="inherit" sx={{ mr: 5 }}>FAQ</Button>
					</Box>
